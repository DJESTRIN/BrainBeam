...
for sample in $scratch_stitch*/; do
	sbatch --job-name=precomputed_volume \
		--mem=200G \
		--partition=scu-cpu \
		--mail-type=BEGIN,END,FAIL \
		--mail-user=dje4001@med.cornell.edu \
		--wrap="bash ./cloudreg.sh '$sample' '$scratch_directory'"
done

sbatch --mem=5G \
	--partition=scu-cpu \
	--dependency=singleton \
	--job-name=precomputed_volume \
	--wrap="bash registration_spinup.sh '$code_directory' '$scratch_directory' '$store_finish_directory'"
