...
class API(DigestData):
    ...
    def _run(self, command):
        """ Run a shell command, raising PipelineError on failure. """
    ...
    def copy(self, input_dir, output_dir):
        ...
        if self.computertype == 'slurm':
            command = f'sbatch --parsable --job-name=gui_copy --mem=300G --partition=scu-cpu --wrap="{command}"'
        return self._run(command)

    def move(self, input_dir, output_dir):
        ...
        if self.computertype == 'slurm':
            command = f'sbatch --parsable --job-name=gui_move --mem=300G --partition=scu-cpu --wrap="{command}"'
        return self._run(command)

    def compress(self, input_dir):
    ...
    # ------------------------------------------------------------------
    def denoise(self, scratch_directory, store_finish_directory=None):
        ...
        if self.computertype == 'slurm':
            ...
            command = (f'sbatch --job-name=gui_denoise --mem=50G --partition=scu-cpu '
    ...
    # Stitch
    # ------------------------------------------------------------------
    def stitch(self, scratch_directory, chain_next_stage=False):
        ...
        if self.computertype == 'slurm':
            ...
            command = (f'sbatch --job-name=gui_stitch --mem=50G --partition=scu-cpu '
    ...
    # Segmentation
    # ------------------------------------------------------------------
    def segment(self, scratch_directory, ilastik_project_file=None):
        ...
        if self.computertype == 'slurm':
            ...
            for script_name, args in steps:
                ...
                results.append(self._run(
                    f'sbatch --job-name=gui_segment --mem=100G --partition=scu-cpu --wrap="bash \'{script}\' {quoted_args}"'))
            return results
