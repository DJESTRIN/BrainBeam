...
do
TMP=$(echo $i)
sbatch --job-name=stitch_files --mem=200G --partition=scu-gpu --gres=gpu:1 --mail-type=BEGIN,END,FAIL --mail-user=dje4001@med.cornell.edu --wrap="bash ./estrin_stitch.sh '$TMP' '$scratch_directory'"
...
# Begin downsampling stitched stack for syGlass ---- NO LONGER NEEDED ----
